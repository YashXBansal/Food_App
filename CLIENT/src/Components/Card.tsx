...
const Card: React.FC<CardProps> = ({ img, description, categoryName, name }) => {
  ...
  const [portion, setPortion] = useState<string>('full');
  // const gstRate = 0.18; // GST rate of 18%
  // const price = 100; // Assuming a fixed price of $100
  // const [totalPrice, setTotalPrice] = useState<number>(price * (1 + gstRate));
  ...
  return (
    <div className="bg-black shadow-md rounded-md p-4 w-64">
      <img src={img} alt={name} className="w-full h-32 object-cover mb-4 rounded-md" />
      <h2 className="text-lg font-semibold mb-2 text-white">{name}</h2>
      <p className="text-gray-400 mb-2">{description}</p>
      <p className="text-gray-400 mb-2">Category: {categoryName}</p>
      <div className="flex items-center mb-2">
        <label htmlFor="quantity" className="mr-2 text-gray-300">Quantity:</label>
        <input
          id="quantity"
